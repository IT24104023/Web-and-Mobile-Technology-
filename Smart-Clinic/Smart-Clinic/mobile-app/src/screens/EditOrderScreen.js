...
export default function EditOrderScreen({ route, navigation }) {
    ...
    axios.get(`${API_URL}/orders/${orderId}`)
      .then(res => {
        ...
        if (['DELIVERED', 'CANCELLED', 'DISPATCHED', 'PROCESSING'].includes(order.orderStatus) || order.paymentStatus === 'PAID') {
          Alert.alert('Cannot Edit', 'This order cannot be edited.');
          navigation.goBack();
          return;
        }
        ...
        const updatedItems = MEDICATIONS.map(med => {
          const match = (order.items || []).find(i => i.medicationName === med.name);
          return { ...med, selected: !!match, quantity: match ? match.quantity : 1 };
        });
  ...
  const handleSave = () => {
    ...
    axios.put(`${API_URL}/orders/${orderId}`, payload)
      .then(res => {
        setSaving(false);
        navigation.navigate('Checkout', { orderId: res.data._id, amount: res.data.totalAmount });
      })
      .catch(err => {
        setSaving(false);
  ...
  if (loading) return <ActivityIndicator size="large" color="#0ea5e9" style={{ marginTop: 50 }} />;
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.summaryCard}>
        ...
        <TouchableOpacity style={styles.saveBtn} onPress={handleSave} disabled={saving}>
          {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>Save & Proceed to Checkout</Text>}
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
